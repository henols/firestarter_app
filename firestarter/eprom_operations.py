...
class EpromOperator:
    ...
    def _get_eprom_data_for_operation(self, eprom_name: str) -> dict | None:
        ...
        # 'can-erase' from db.get_eprom(full=True) would be used to set FLAG_CAN_ERASE
        # but db.get_eprom(full=False) (default) doesn't include 'can-erase'.
        # The 'flags' field from JSON should already account for this.
        # If FLAG_CAN_ERASE needs to be dynamically set based on 'can-erase' even for concise data,
        # we'd need to fetch full data or ensure JSON 'flags' are comprehensive.
        # For now, assume JSON 'flags' are sufficient.

        logger.debug(f"EPROM data for {eprom_name} prepared for operation.")
        return eprom_data

    def _calculate_buffer_size(self) -> int:
        if self.comm and self.comm.programmer_info and "leonardo" in self.comm.programmer_info.lower():
            return LEONARDO_BUFFER_SIZE
        return BUFFER_SIZE

    def _setup_operation(self, eprom_name: str, state: int, operation_flags: int = 0, 
                         address_str: str | None = None, size_str: str | None = None) -> tuple[dict | None, int]:
        """
        ...
        if address_str:
            try:
                command_dict["address"] = int(address_str, 16) if "0x" in address_str.lower() else int(address_str)
            except ValueError:
                logger.error(f"Invalid address format: {address_str}")
                return None, 0
        
        # Special handling for read operation size
        if state == STATE_READ and size_str:
            try:
                read_size = int(size_str, 16) if "0x" in size_str.lower() else int(size_str)
        ...
        try:
            ...
            logger.debug(f"Operation setup for {eprom_name} (state {state}) complete ({time.time() - start_time:.2f}s). Buffer size: {buffer_size}")
    ...
    def _perform_simple_command(self, eprom_name: str, state: int, operation_flags: int = 0, success_log_msg: str = "") -> bool:
        command_eprom_data, _ = self._setup_operation(eprom_name, state, operation_flags)
        if not command_eprom_data or not self.comm:
            return False

        start_time = time.time()
        operation_name = [k for k,v in globals().items() if v == state][0].replace("STATE_", "") # Get state name
        logger.info(f"{success_log_msg or f'Performing {operation_name} for {eprom_name.upper()}'}")
        ...
        try:
            ...
            if not is_ok:
                ...
                return False
            logger.info(f"{operation_name} for {eprom_name.upper()} successful ({time.time() - start_time:.2f}s). {msg or ''}")
            return True
    ...
    def _read_data_from_programmer(self, buffer_size: int) -> bytes | None:
        if not self.comm: return None
        try:
            response_type, message = self.comm.get_response() # Wait for DATA: or OK: or ERROR:
            if response_type == "DATA":
                ...
                logger.error(f"Unexpected response or timeout during data read: {response_type} - {message}")
                raise EpromOperationError("Timeout or unexpected response during data read.")
        except (SerialError, SerialTimeoutError) as e:
            logger.error(f"Serial communication error during data read: {e}")
            raise EpromOperationError(f"Serial error: {e}") from e

    def _send_file_to_programmer(self, eprom_data_for_command: dict, input_file_path: str, buffer_size: int) -> bool:
        if not self.comm: return False
    ...
    def read_eprom(self, eprom_name: str, output_file: str | None = None, 
                   operation_flags: int = 0, address_str: str | None = None, size_str: str | None = None) -> bool:
        
        command_eprom_data, buffer_size = self._setup_operation(eprom_name, STATE_READ, operation_flags, address_str, size_str)
        if not command_eprom_data or not self.comm:
        ...
        try:
            with open(actual_output_file, "wb") as file_handle:
                ...
                end_addr = command_eprom_data.get("memory-size", 0) # This is end-address for read
                total_bytes_to_read = end_addr - start_addr
                if total_bytes_to_read <= 0:
                    logger.error(f"Invalid read range: start {start_addr}, end {end_addr}")
                    return False
    ...
    def dev_set_registers(self, msb_str: str, lsb_str: str, ctrl_reg_str: str, operation_flags: int = 0) -> bool:
        try:
            msb = int(msb_str, 16) if "0x" in msb_str.lower() else int(msb_str)
            lsb = int(lsb_str, 16) if "0x" in lsb_str.lower() else int(lsb_str)
            ctrl_reg = int(ctrl_reg_str, 16) if "0x" in ctrl_reg_str.lower() else int(ctrl_reg_str)
        except ValueError:
            logger.error("Invalid register value format. Must be decimal or hex (e.g., 0xFF).")
            return False
        ...
        command_dict_for_connect = {"state": STATE_DEV_REGISTERS, "flags": operation_flags}
        try:
    ...
    def dev_set_address_mode(self, eprom_name: str, address_str: str, operation_flags: int = 0) -> bool:
        # This command sets the RURP into a mode where it holds a specific address
        # based on the EPROM's pin map.
        command_eprom_data, _ = self._setup_operation(eprom_name, STATE_DEV_ADDRESS, operation_flags, address_str)
        if not command_eprom_data or not self.comm:
        ...
        logger.info(f"Developer Mode: RURP address set to 0x{command_eprom_data.get('address', 0):06X} using {eprom_name.upper()}'s pin map.")
        self._disconnect_programmer() # Disconnect after setting the mode.
        return True

    def write_eprom(self, eprom_name: str, input_file_path: str, operation_flags: int = 0, address_str: str | None = None) -> bool:
        command_eprom_data, buffer_size = self._setup_operation(eprom_name, STATE_WRITE, operation_flags, address_str)
        if not command_eprom_data or not self.comm:
            return False
        ...
        try:
            ...
            success = self._send_file_to_programmer(command_eprom_data, input_file_path, buffer_size)
            if success:
    ...
    def verify_eprom(self, eprom_name: str, input_file_path: str, operation_flags: int = 0, address_str: str | None = None) -> bool:
        command_eprom_data, buffer_size = self._setup_operation(eprom_name, STATE_VERIFY, operation_flags, address_str)
        if not command_eprom_data or not self.comm:
            return False
    ...
    def erase_eprom(self, eprom_name: str, operation_flags: int = 0) -> bool:
        return self._perform_simple_command(eprom_name, STATE_ERASE, operation_flags, 
                                            f"Erasing EPROM {eprom_name.upper()}")

    def check_eprom_blank(self, eprom_name: str, operation_flags: int = 0) -> bool:
        return self._perform_simple_command(eprom_name, STATE_CHECK_BLANK, operation_flags,
                                            f"Performing blank check for {eprom_name.upper()}")

    def check_eprom_id(self, eprom_name: str, operation_flags: int = 0) -> bool:
        command_eprom_data, _ = self._setup_operation(eprom_name, STATE_CHECK_CHIP_ID, operation_flags)
        if not command_eprom_data or not self.comm:
            return False
        ...
        try:
            self.comm.send_ack() # Tell programmer to proceed
            is_ok, message = self.comm.expect_ok() # Message contains chip ID if OK, or error

            if is_ok:
                logger.info(f"Chip ID check passed for {eprom_name.upper()}: {message} ({time.time() - start_time:.2f}s)")
                return True
            else:
                # If not OK, the message might be an error or a different detected ID
                logger.warning(f"Chip ID check for {eprom_name.upper()} did not return OK. Programmer response: {message}")
                ...
                if chip_id_decimal is not None:
                    ...
                    if found_eproms:
                        logger.info(f"The reported Chip ID 0x{chip_id_decimal:X} matches the following EPROMs in the database:")
                        # Map raw data from search_chip_id to the format expected by print_eprom_list_table
                        mapped_found_eproms = [self.db._map_data(ic, ic.get("manufacturer", "Unknown")) for ic in found_eproms]
                        ...
                        logger.warning(f"Reported Chip ID 0x{chip_id_decimal:X} not found in the database.")
                else:
                    logger.error(f"Failed to extract a valid chip ID from programmer response: {message}")
    ...
    def dev_read_eprom(self, eprom_name: str, address_str: str | None = None, size_str: str = "256", operation_flags: int = 0) -> bool:
        ...
        command_eprom_data, buffer_size = self._setup_operation(eprom_name, STATE_READ, # Dev read uses normal read state
                                                                operation_flags, address_str, size_to_read_str)
        if not command_eprom_data or not self.comm:
            return False
        ...
        logger.info(f"Developer Read: Reading {num_bytes_to_dev_read} bytes from address 0x{start_addr_for_read:04X} of {eprom_name.upper()}")
        start_time = time.time()
        ...
        try:
            ...
            while bytes_read_total < num_bytes_to_dev_read:
                response_type, message = self.comm.get_response()
                if response_type == "DATA":
                    data_chunk = self.comm.read_data_block(buffer_size) # RURP sends buffer_size chunks
