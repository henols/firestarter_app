...
from firestarter.serial_comm import SerialCommunicator, ProgrammerNotFoundError, SerialError
...
class FirmwareManager:
    ...
    def check_current_firmware(self, preferred_port: str | None = None) -> tuple[str | None, str | None, str | None]:
        """
        ...
        command_dict = {"state": STATE_FW_VERSION}
        comm = None
        try:
            ...
            if comm.programmer_info:
                ...
                logger.error("Failed to read firmware version: No valid response from programmer.")
    ...
    def fetch_latest_release_info(self, board: str = "uno") -> tuple[str | None, str | None]:
        """
        ...
        try:
            ...
            if not latest_version or not download_url:
                logger.error(f"Could not find firmware version or URL for board '{board}' in the latest release.")
                return None, None
            
            logger.debug(f"Latest firmware version for {board}: {latest_version}, URL: {download_url}")
            return latest_version, download_url
        except requests.RequestException as e:
            logger.error(f"Failed to fetch latest firmware release information: {e}")
            return None, None

    def _compare_versions(self, current_version_str: str | None, latest_version_str: str | None) -> bool:
        ...
        try:
            ...
            logger.warning(f"Could not parse version strings for comparison: '{current_version_str}', '{latest_version_str}'")
    ...
    def _download_firmware_file(self, url: str) -> str | None:
        ...
        try:
            ...
            filename = url.split('/')[-1] if url.split('/')[-1] else "firestarter_downloaded.hex"
            firmware_path = os.path.join(HOME_PATH, filename)

            with open(firmware_path, "wb") as f:
                for chunk in response.iter_content(chunk_size=8192):
                    f.write(chunk)
            logger.info(f"Firmware downloaded to: {firmware_path} ({time.time() - start_time:.2f}s)")
    ...
    def _install_with_avrdude(self, hex_file_path: str, board: str, 
                               avrdude_path_override: str | None, 
                               avrdude_config_override: str | None, 
                               target_port: str | None) -> bool:
        """Internal method to perform the Avrdude flashing process."""
        start_time = time.time()
        
        partno, programmer_id, baud_rate = ("atmega328p", "arduino", 115200) # Defaults for uno
        ...
        if not ports_to_try and not target_port: # If target_port was None and no saved port
            logger.error("No specific port provided for Avrdude and no port saved in config. Please specify a port.")
            ...
            if not target_port: # This check is redundant if the logic above is followed.
                 logger.error("Target port for Avrdude is unknown.")
                 return False
            ports_to_try.append(target_port) # Should be set by caller

        avrdude_path = avrdude_path_override or self.config_manager.get_value("avrdude-path")
        avrdude_config_path = avrdude_config_override or self.config_manager.get_value("avrdude-config-path")

        for port_to_flash in ports_to_try: # Usually, this will be a single port.
            logger.info(f"Attempting to flash firmware to {board} on port {port_to_flash} using Avrdude...")
            try:
                avrdude = Avrdude(
                    partno=partno, programmer_id=programmer_id, baud_rate=baud_rate, port=port_to_flash,
                    avrdude_path=avrdude_path, avrdude_config_path=avrdude_config_path
                )
                
                stderr_output, return_code = avrdude.flash_firmware(hex_file_path)
                if return_code == 0:
                    logger.info(f"Firmware successfully updated on {port_to_flash} ({time.time() - start_time:.2f}s)")
                    self.config_manager.set_value("port", port_to_flash) # Save successful port
                    if avrdude_path_override is None: # Only save if not overridden by user for this run
                        self.config_manager.set_value("avrdude-path", avrdude.command)
                    if avrdude_config_override is None and avrdude.config:
                         self.config_manager.set_value("avrdude-config-path", str(avrdude.config.absolute()))
                    return True
                else:
                    logger.error(f"Firmware update failed on port {port_to_flash}. Avrdude stderr:")
            ...
            except Exception as e: # Catch other potential errors from Avrdude instantiation/execution
                logger.error(f"An unexpected error occurred with Avrdude on port {port_to_flash}: {e}")
    ...
    def manage_firmware_update(self, install_flag: bool = False, 
        ...
        if not current_version and not install_flag and not force_install:
            logger.error("Could not determine current firmware version. Use --install or --force to proceed with installation.")
        ...
        if is_up_to_date and not force_install:
            logger.info(f"Firmware is already up to date (version {current_version} for {board_to_use}). Use --force to reinstall.")
        ...
        if force_install:
            ...
            if not is_up_to_date:
                logger.info(f"Proceeding with firmware update for {board_to_use} (current: {current_version}, latest: {latest_version}).")
                should_install_now = True
            else: # install_flag is true, but already up-to-date
                logger.info(f"Firmware for {board_to_use} is already version {latest_version}. Use --force to reinstall.")
                # No installation needed unless forced
        elif not is_up_to_date and current_version and latest_version : # No flags, but update available
            if Confirm.ask(f"New firmware {latest_version} available for {board_to_use} (current: {current_version}). Update now?", default=False):
                should_install_now = True
            else:
                logger.info("Firmware update cancelled by user.")
        elif not current_version and (install_flag or force_install): # No current version, but user wants to install
             logger.info(f"No current firmware version detected. Proceeding with installation of latest version for {board_to_use}.")
             should_install_now = True

        if should_install_now:
            if not download_url or not latest_version:
                logger.error(f"Cannot install: latest firmware URL or version for {board_to_use} is not available.")
                return False
            ...
            if os.path.exists(hex_file):
                try:
                    ...
                    logger.warning(f"Could not remove temporary firmware file {hex_file}: {e}")
