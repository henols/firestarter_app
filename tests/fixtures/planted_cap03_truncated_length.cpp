 ...
 * with exactly ONE planted change: the emitted byte count below is
 * `(uint8_t)(4 + _vlen)`, not the real firmware's `(uint8_t)(4 + _vlen +
 * 2)`. The two CAP-03 budget bytes are still PACKED into `_ready` at the
 * correct computed offset, but the emit's length argument omits them -- a
 * SILENT capability loss: the host's write_block_budget_s attribute would
 * stay None forever, with no error anywhere on either side of the wire.
 *
 * This fixture deliberately does NOT trip:
 *   - the computed-offset leg
 *     (test_budget_is_written_and_read_at_the_computed_offset) -- the
 *     budget bytes are still written at `4 + _vlen` / `4 + _vlen + 1`
 *   - the index-identity leg for bytes 0-3 -- untouched
 *   - the big-endian leg (test_both_sides_use_big_endian_for_both_u16_fields)
 *     -- both `>> 8` / `& 0xFF` shapes are unchanged
 ...
 *
 * "Fixing" this file (i.e. adding the `+ 2` back) would silently hollow
 * this gate's emitted-length detection leg -- the silent-capability-loss
 * defect class this gate exists to catch. Do NOT "fix" this file. If the
 * real firmware's `_ready` pack region ever changes shape, update this
 * fixture to match the new shape (keeping exactly one planted
 * truncated-length drift), do not delete the violation.
 */
...

    {
        ...
        LOG_OK_ID_BYTES(MSG_OK_READY, _ready, (uint8_t)(4 + _vlen));  /* PLANTED VIOLATION -- real firmware emits (uint8_t)(4 + _vlen + 2) */
    }
